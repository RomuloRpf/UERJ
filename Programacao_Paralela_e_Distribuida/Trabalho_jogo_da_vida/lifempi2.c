...
void play (cell_t ** board, cell_t ** newboard, int size, int numtasks, int rank, int flag) {
	int	i, j, a, chuck, local_init, local_end, index, index_e, tag = 0, flag2 = 0;
	MPI_Status Stat;
	...
	for (i = local_init; i < local_end; i++) {
		...
		if((flag == 1) && (chuck > 2)){
			...
			flag2 = 1;
			if(rank != (numtasks-1)){
				if((i != local_init) && (i != (local_end-1))){
					//tag = rank;
					index = i;
					MPI_Send(&index, 1, MPI_INT, (numtasks-1), tag, MPI_COMM_WORLD);
					MPI_Send(newboard[index], size, MPI_UNSIGNED_CHAR, (numtasks-1), tag, MPI_COMM_WORLD);
				}
			}
			else if(flag2 == 1){
				//for(x=0; x < (numtasks-1); x++){
					//tag = x;
					//printf("tag = %d\n", tag);
					for(j=0; j < (chuck-2)*(numtasks-1); j++){
						MPI_Recv(&index, 1, MPI_INT, MPI_ANY_SOURCE, tag, MPI_COMM_WORLD, &Stat);
						MPI_Recv(newboard[index], size, MPI_UNSIGNED_CHAR, MPI_ANY_SOURCE, tag, MPI_COMM_WORLD, &Stat);
						//printf("j = %d\n", j);
					}
					//printf("\n");
				//}
...
void read_file (FILE * f, cell_t ** board, int size) {
	...
	for (j=0; j<size; j++) {
		...
		for (i=0; i<size; i++)
		{
			...
			board[i][j] = s[i] == 'x';
			if(j == (size-1)){
				MPI_Bcast (board[i], size, MPI_UNSIGNED_CHAR, 0, MPI_COMM_WORLD);
			}
		}
		//fscanf(f,"\n");
	}
...
int main(int argc, char *argv[]) {
	...
	for (i=0; i<steps; i++) {
		...
		//if(rank == 3) print(next,size,rank);
		#ifdef DEBUG
			printf("%d ----------\n", i);
			print (next,size);
		#endif
